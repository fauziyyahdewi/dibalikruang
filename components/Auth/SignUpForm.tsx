...
import TermsAgreementButton from "@/components/ui/termsagreementbutton";
import Link from "next/link";
import React from "react";
import { signUpCredentials } from "@/lib/actions";
import Swal from "sweetalert2";
import { useForm } from "react-hook-form";
import { FormDataRegist } from "@/types/form-values";
...
const SignUpForm = () => {
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<FormDataRegist>();

  const onSubmit = async (data: FormDataRegist) => {
    ...
    if (result?.error) {
      Object.entries(result.error).forEach(([key, value]) => {
        setError(key as keyof FormDataRegist, {
          type: "server",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-1">
        <TermsAgreementButton label="CREATE ACCOUNT" />
