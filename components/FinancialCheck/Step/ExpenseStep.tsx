...
function ExpenseStep({ form }: { form: UseFormReturn<FormValues> }) {
  const { control } = form;
  const { fields, append, remove } = useFieldArray({
    control,
    name: "expensesSources",
  });
  ...
  return (
    <>
      ...
      <div className="bg-amber-50 rounded-lg p-4 mb-8">
        {fields.length > 0 && (
          <h3 className="text-lg font-medium mb-4">Ada pengeluaran lainnya?</h3>
        )}
        ...
        {showCustomInput && (
          <div className="mt-4 p-4 bg-white rounded-lg border border-amber-200">
            <label className="block text-sm text-gray-600 mb-2">
              Masukkan jenis pengeluaran lainnya:
            </label>
            <div className="flex gap-2">
              <Input
                ...
                className="flex-1"
                placeholder="Contoh: Donasi, Cicilan, dll."
              />
              <Button
                type="button"
